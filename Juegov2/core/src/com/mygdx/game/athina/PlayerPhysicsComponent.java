...
public class PlayerPhysicsComponent extends PhysicsComponent {
	private static final String TAG = PlayerPhysicsComponent.class
			.getSimpleName();
	...
	public PlayerPhysicsComponent() {
		//cambiar los siguienes 2 offsets y meterlos en el json
		offsetX = 0;
		offsetY = -10;
	...
	public void receiveMessage(String message) {
		...
		if (string.length == 2) {
			if (string[0].equalsIgnoreCase(MESSAGE.INIT_START_POSITION
				...
				nextEntityPosition.set(currentEntityPosition.x,
						currentEntityPosition.y);
			} else if (string[0].equalsIgnoreCase(MESSAGE.CURRENT_STATE
					.toString())) {
				state = json.fromJson(Entity.State.class, string[1]);
			} else if (string[0].equalsIgnoreCase(MESSAGE.CURRENT_DIRECTION
					.toString())) {
				currentDirection = json.fromJson(Entity.Direction.class,
						string[1]);
			} else if (string[0].equalsIgnoreCase(MESSAGE.INIT_SELECT_ENTITY
					.toString())) {
				mouseSelectCoordinates = json
						.fromJson(Vector3.class, string[1]);
	...
	private void selectMapEntityCandidate(MapManager mapMgr) {
		...
		for (Entity mapEntity : currentEntities) {
			...
			if (mapEntity.getCurrentBoundingBox().contains(
				...
				// Check distance
				selectionRay.set(boundingBox.x, boundingBox.y, 0.0f,
						mapEntityBoundingBox.x, mapEntityBoundingBox.y, 0.0f);
				float distance = selectionRay.origin
						.dst(selectionRay.direction);
	...
	public void update(Entity entity, MapManager mapMgr, float delta) {
		...
		if (!isCollisionWithMapLayer(entity, mapMgr)
			...
			camera.position.set(currentEntityPosition.x,
					currentEntityPosition.y, 0f);
			camera.update();
		} else {
