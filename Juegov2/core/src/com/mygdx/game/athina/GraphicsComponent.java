...
public abstract class GraphicsComponent implements Component {
	...
	protected void updateAnimations(float delta) {
		...
		case DOWN:
			if (currentState == Entity.State.WALKING) {
				Animation animation = animations
						.get(Entity.AnimationType.WALK_DOWN);
				if (animation == null)
					return;
				...
				Animation animation = animations
						.get(Entity.AnimationType.IMMOBILE);
				if (animation == null)
					return;
				currentFrame = animation.getKeyFrame(frameTime);
			}
		...
		case LEFT:
			if (currentState == Entity.State.WALKING) {
				Animation animation = animations
						.get(Entity.AnimationType.WALK_LEFT);
				if (animation == null)
					return;
				currentFrame = animation.getKeyFrame(frameTime);
			} else if (currentState == Entity.State.IDLE) {
		...
		case UP:
			if (currentState == Entity.State.WALKING) {
				Animation animation = animations
						.get(Entity.AnimationType.WALK_UP);
				if (animation == null)
					return;
				currentFrame = animation.getKeyFrame(frameTime);
			} else if (currentState == Entity.State.IDLE) {
		...
		case RIGHT:
			...
			if (currentState == Entity.State.WALKING) {
				Animation animation = animations
						.get(Entity.AnimationType.WALK_RIGHT);
	...
	protected Animation loadAnimation(String firstTexture,
			String secondTexture, Array<GridPoint2> points, float frameDuration) {
		...
		TextureRegion[][] texture1Frames = TextureRegion.split(texture1,
				Entity.FRAME_WALK_WIDTH, Entity.FRAME_WALK_HEIGHT);
	...
	protected Animation loadAnimation(String textureName,
		...
		TextureRegion[][] textureFrames = TextureRegion.split(texture,
				Entity.FRAME_WALK_WIDTH, Entity.FRAME_WALK_HEIGHT);
		Array<TextureRegion> animationKeyFrames = new Array<TextureRegion>(
				points.size);
		System.out.println("size de points "+points.size );
		for (GridPoint2 point : points) {
			animationKeyFrames.add(textureFrames[point.y][point.x]); //ATENTION I DONT KNOW WHY Y AND X SWITCHED
		}
		return new Animation(frameDuration, animationKeyFrames,
				Animation.PlayMode.LOOP);
	}
